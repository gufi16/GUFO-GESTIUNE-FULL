...
export async function restoreTenantBackupFromFile(tenantId: string, filePath: string) {
  ...
  const locations = asArray(payload.locations).map((item) =>
    pickFields(item, ["id", "tenantId", "companyId", "name", "code", "address", "street", "streetNo", "building", "staircase", "floor", "apartment", "details", "city", "county", "country", "postalCode", "isActive", "createdAt", "updatedAt"]),
  )
